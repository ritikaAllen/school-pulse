...
description: |
  ...
  Do NOT use for trend analysis, memory updates, or PII sanitization â€” those
...
- Task is to compute trend or baseline â€” use `student-trend-tracker` instead
- Task is to generate the counselor brief â€” that belongs to the Orchestrator
...
> by `pii-context-sanitizer`. This skill cannot verify provenance â€” sanitization order
...
### Step 1 â€” Detect input modality
...
- Set `social_withdrawal_flag = true` if ðŸ˜¶ ðŸ™ˆ ðŸ˜¶â€ðŸŒ«ï¸ or equivalent isolation emoji present
- `distress_keywords_detected = []` (emoji path does not produce keywords)
- `signal_confidence`: 1.0 if sequence â‰¥ 2 emojis; 0.6 if single emoji
...
- `social_withdrawal_flag`: true if response contains isolation language â€”
...
- `signal_confidence`: 0.9 if response â‰¥ 15 words; 0.6 if 5â€“14 words; 0.3 if < 5 words
...
### Step 3 â€” Merge signals (if multiple modalities present)
...
### Step 4 â€” Emit structured signal output
...
**Hard rules:**
- Never infer student name â€” `student_id` must come directly from the input record
- Never produce `signal_confidence > 0.95` â€” no single check-in warrants certainty
- If input is malformed or empty, return `signal_confidence = 0.0` and log the anomaly
- Do not escalate or set priority â€” that is the Memory Keeper's job
...
- Do NOT set `recommended_priority` â€” this field belongs to the trend report, not the signal
- Do NOT look up prior days' signal history â€” this skill processes within a single
  ...
  record for the same date â€” it does not reach backwards into previous days.
- Do NOT pass raw student name or PII into any reasoning step â€” input must already be sanitized
...
[
  {
    ...
    "input": {
      ...
      "age_group": "junior",
      "date": "2026-06-28",
      "junior_input": { "emoji_sequence": "ðŸ˜”ðŸ˜´ðŸ˜ " }
    },
    "expected_output": {
  ...
  },
  {
    ...
    "description": "Junior student with positive emoji â€” no false escalation",
    "input": {
      "student_id": "S_011",
      "age_group": "junior",
      "date": "2026-06-28",
      "junior_input": { "emoji_sequence": "ðŸ˜ŠðŸ˜„ðŸŒŸ" }
    },
    "expected_output": {
...
- `references/emoji_affect_table.md` â€” full emoji â†’ (valence, energy) lookup table
- `../student-trend-tracker/SKILL.md` â€” next step after signal is produced
- `../pii-context-sanitizer/SKILL.md` â€” must run BEFORE this skill on raw input
- `specs/signal-detector.md` â€” sub-agent spec this skill belongs to
