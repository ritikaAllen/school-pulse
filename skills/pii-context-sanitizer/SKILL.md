...
description: |
  ...
  Implements three context hygiene techniques: (1) PII masking — replace student
  names with anonymised IDs, redact contact info and named entities from free text;
  (2) context windowing — enforce a 7-day max history, drop oldest first;
  (3) memory boundary enforcement — block cross-student record access and counselor
...
passing through this skill first. Its job is not analysis — it is boundary
...
- Record has already been sanitised in the current pipeline run — do not double-sanitise
- Task is signal parsing — use `emotional-signal-reader` after sanitisation
- Task is trend computation — use `student-trend-tracker` after sanitisation
- Task is to evaluate output quality — that belongs to the LLM-as-judge layer
...
> Orchestrator's routing logic — not in downstream skill guards.
...
scoped to a different type of privacy risk. They do not interact — run all three
on every record.
...
`student_id` remains. This skill does not own the name→ID mapping — it receives
the registry from the Orchestrator as part of its input context.

#### 1b. Teacher observation notes — named entity redaction
...
structured fields. Replace every detected PERSON entity — including student
names, peer names, family member names. Do not attempt to identify *which*
person the name refers to.

#### 1c. Senior free-text responses — contact identifier redaction
...
rule:
  ...
  patterns:
    - email_addresses:   regex → [REDACTED_EMAIL]
    - phone_numbers:     regex → [REDACTED_PHONE]
    - physical_addresses: NER → [REDACTED_ADDRESS]
    - social_handles:    regex → [REDACTED_HANDLE]
  example:
...
3. Return the trimmed record; do not modify the memory store itself —
...
### Pass 3 — Memory Boundary Enforcement
...
- **Reject the payload** — do not sanitise and pass through
...
Counselor names must not appear in any LLM context window — their identity
is irrelevant to signal analysis and creates unnecessary data exposure.

---

### Output — sanitised record
...
- A manifest with `boundary_checks_passed: false` must halt the pipeline —
...
- If NER or regex passes fail technically, return a `sanitisation_error` —
...
- Do NOT pass a record downstream if `boundary_checks_passed = false` —
  a boundary violation must halt, not warn-and-continue
- Do NOT attempt to infer which PERSON entity is the subject student —
  replace all PERSON entities uniformly
- Do NOT modify the `student_memory` store during context windowing —
  Pass 2 produces a trimmed *view* for the context payload; the authoritative
  memory write belongs to `student-trend-tracker`
- Do NOT cache or store the name→ID mapping — this skill receives it as
...
[
  {
    ...
    "description": "Teacher note containing two student names — both must be replaced",
    "input": {
      "student_id": "S_042",
      "teacher_observation": {
        "flag_level": "concern",
  ...
  },
  {
    ...
    "description": "Senior response containing email address — redacted, emotional content preserved",
    "input": {
      "student_id": "S_004",
      "senior_input": {
        "prompt": "How are you feeling today and why?",
  ...
  },
  {
    ...
    "description": "Teacher note with no PII — passes through unchanged",
    "input": {
      "student_id": "S_011",
      "teacher_observation": {
        "flag_level": "none",
...
- `references/pii_redaction_patterns.md` — regex patterns for contact identifier detection
- `../emotional-signal-reader/SKILL.md` — runs immediately after this skill
- `../student-trend-tracker/SKILL.md` — runs after emotional-signal-reader
- `specs/privacy-guard.md` — sub-agent spec this skill belongs to
