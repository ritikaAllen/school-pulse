...
- Orchestrator requests a trend report for a student without a new signal today
  (read-only mode — emit report from existing memory, no update)

## When NOT to use

- Input is a raw check-in record, not a structured `signal` — run `emotional-signal-reader` first
- Task is PII sanitization — use `pii-context-sanitizer`
- Request involves more than one student in a single call — process each student separately
- Request asks to compare two students' records — cross-student access is forbidden
...
### Step 1 — Load or initialise student memory
...
### Step 2 — Integrate new signal into rolling window
...
- Each entry is uniquely identified by `date` — if today's date already exists in
...
the counter — the system does not carry forward low-day counts across recovery days.
...
`crisis_watch` takes precedence — if the crisis condition is met, trend is
`crisis_watch` regardless of the valence trajectory.

---

### Step 7 — Set recommended priority
...
### Step 8 — Write updated memory and emit trend report
...
**Hard rules:**
- Never emit a `trend_report` containing the student's name — `student_id` only
...
- Do not write to memory if signal integration fails — emit error, leave memory intact
...
- Do NOT read another student's memory record — each call is scoped to exactly one `student_id`
- Do NOT average today's signal into the baseline before appending it to history —
  compute baseline from history first, then append, to avoid circular dependency
- Do NOT reset `consecutive_low_days` when writing memory if today was a low day —
  only reset on a non-low day
- Do NOT set `pattern_break_detected = true` when baseline is null — insufficient
  history means no reliable baseline to break from
- Do NOT carry distress keywords or raw text into the trend report — the report
...
[
  ...
  },
  {
    ...
    "description": "Student with consistently positive history receives another positive signal — no escalation",
    "input": {
      "student_id": "S_011",
      "existing_signal_history": [
...
- `references/priority_decision_tree.md` — visual decision tree for priority rules
- `../emotional-signal-reader/SKILL.md` — upstream skill; must run before this one
- `../pii-context-sanitizer/SKILL.md` — sanitization; must run before both skills
- `specs/memory-keeper.md` — sub-agent spec this skill belongs to
